...
Определите переменную окружения `SECRET_KEY` и `YANDEX_GEOCODER_API_KEY`- необходим для работы с API Яндекс геокодера(получение координат адреса) (ВАЖНО!!!). Создать файл `.env` в каталоге `star_burger/` и положите туда такой код:
```sh
...
- `YANDEX_GEOCODER_API_KEY` - [можно получить по ссылке](https://developer.tech.yandex.ru/services). Необходим для работы с API Яндекс геокодера(Получение координат аддреса)
